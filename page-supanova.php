			...
			<div class="entry-content col-xs-12">
				...
				<div class="row">
					...
					<div class="stock-tally col-xs-6 col-sm-4 col-md-2">
						<!-- Dolls in Stock -->
						<a id="stock_level" class="btn btn-default btn-success btn-block btn-stock" href="#"><span class="i"><?= $total; ?></span><br>Dolls in Stock Currently</a>
					</div>

					<div class="stock-tally col-xs-6 col-sm-4 col-md-2">
						<a id="max_value" class="btn btn-default btn-info btn-block btn-stock" href="#"><span class="i">$<?= $possible_profit; ?></span><br>Maximum Possible Value
						</a>
					</div>

					<div class="stock-tally col-xs-6 col-sm-4 col-md-2">
						<a id="dolls_remaining" class="btn btn-default <?= $ideal > $days_to_supanova ? 'btn-danger' : 'btn-success'; ?> btn-block btn-stock" href="#"><span class="i"><?= $ideal; ?></span><br>Dolls Remaining</a>
					</div>

					<div class="stock-tally col-xs-6 col-sm-4 col-md-2">
						<a id="proposed_value" class="btn btn-default btn-block btn-info btn-stock" href="#"><span class="i">$<?= $proposed_profit; ?></span><br>Value of Ideal Stock</a>
					</div>

					<div class="stock-tally col-xs-6 col-sm-4 col-md-2">
						<a href="#" class="btn btn-default btn-block btn-warning btn-stock"><span class="i"><?= $days_to_supanova; ?></span><br> Days until Supanova</a>
					</div>

					<div class="stock-tally col-xs-6 col-sm-4 col-md-2">
						<?php $dolls_per_week = $ideal / ($days_to_supanova / 7); ?>
						<?php $dolls_per_day = $ideal / ($days_to_supanova); ?>
						<a id="dolls_per_day" class="btn btn-primary btn-default btn-block btn-stock" href="#"><span class="i" title="<?= round($dolls_per_day, 2); ?>"><?= round($dolls_per_day, 2); ?></span>
							<br>Dolls per Day
						...
						<table id="dolls" class="table table-striped table-hover" cellspacing="0" width="100%">
							...
							<?php foreach ($dolls as $key => $doll) : ?>
								...
								<?php if (!$inactive) : ?>
									...
									<tr class="doll-<?= $doll->ID; ?> <?= $stock >= $ideal_stock ? 'table-success' : ''; ?> <?= $stock < $ideal_stock ? 'table-focus' : ''; ?> <?= $stock == 0 ? 'table-warning' : ''; ?> <?= $on_order > 0 ? 'table-order' : ''; ?>">
										...
										<td class="avg_views" data-toggle="tooltip" data-placement="right" title="Total: <?= $views; ?>"><?= $avg_views; ?></td>
										<td class="avg_favourites" data-toggle="tooltip" data-placement="right" title="Total: <?= $favourites; ?>" data-val="<?= $favourites; ?>"><?= $avg_favourites; ?></td>
